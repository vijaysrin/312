...
int minRec2(int x[], int n) {
	if(n <= 1)
		...
		int lower = minRec2(x, n/2);
		int higher = minRec2(x + n/2, n - n/2);
...
double sqrtRec(double x, double low_guess, double high_guess) {
	double ep = 1E-15;
	if(high_guess - low_guess < ep)
		return high_guess;
	else{
		high_guess = (high_guess + low_guess) / 2;
		low_guess = x / high_guess;
...
int strCompare2(char* str1, char* str2) {
	int one = whatLetter(*str1);
	int two = whatLetter(*str2);
	if(*str1 == '\0' && *str1 == *str2)
		return 0;
	else if(one >= 0 && two >= 0){
		if(one == two)
	...
	else if(one < 0 && (two >= 0 || *str2 == '\0'))
		return strCompare2(str1 + 1, str2);
	else if((one >= 0 || *str1 == '\0') && two < 0)
...
int solveMazeRec(int row, int col) {
	...
	if  (row == MATRIX_SIZE - 1 || 
		(row + 1 < MATRIX_SIZE && solveMazeRec(row + 1, col)) || 
		(col - 1 > 0 && solveMazeRec(row, col - 1)) || 
		(row - 1 > 0 && solveMazeRec(row - 1, col)) || 
		(col + 1 < MATRIX_SIZE && solveMazeRec(row, col + 1))){
		return 1;
	}
	else{
		maze[row][col] = 0;
...
Martian change(int cents) {
	...
	if(cents < 5){
		...
		return man;
		   ...
		   (cents - 12*(cents / 12)) / 5 + 
		   (cents - 12*(cents / 12) - 
		   	5*((cents - 12*(cents / 12)) / 5)) >= 
		    cents / 5 + cents % 5){

		man.pennies = change(cents % 5).pennies;
		man.nicks = cents / 5;
		man.dodeks = 0;
		return man;
	} 
	else{ 
		man.pennies = change(cents % 12).pennies;
		man.nicks = change(cents % 12).nicks;
		man.dodeks = cents / 12;
		return man;
	}
}
...
Martian change(int cents, int nick_val, int dodek_val) {
	...
	if(cents < nick_val){
		...
		return man;
		   ...
		   (cents - dodek_val*(cents / dodek_val)) / nick_val + 
		   (cents - dodek_val*(cents / dodek_val) - 
		   	nick_val*((cents - dodek_val*(cents / dodek_val)) / nick_val)) >= 
		    cents / nick_val + cents % nick_val){

		man.pennies = change(cents % nick_val).pennies;
		man.nicks = cents / nick_val;
		man.dodeks = 0;
		return man;
	} 
	else{ 
		man.pennies = change(cents % dodek_val).pennies;
		man.nicks = change(cents % dodek_val).nicks;
		man.dodeks = cents / dodek_val;
		return man;
	}
}
...
Martian changeIt(int cents) {
	Martian x;
	return x; // delete this line, it's broken. Then write the function properly!
}
